                ...
                {(["breakfast", "lunch", "dinner"] as const).map((m) => {
                  ...
                  return (
                    <div key={m} className="bg-card p-4">
                      <div className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">{m}</div>
                      {r ? (
                        <Link to="/recipes/$id" params={{ id: r.id }} className="mt-1 block text-sm font-medium hover:text-primary">
                          {r.name}
                        </Link>
                      ) : (
